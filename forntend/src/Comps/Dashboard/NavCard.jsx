...
// ─────────────────────────────────────────────────────────
// כרטיס ניווט ראשי — מקשר לעמוד אחר.
// מקבל צבע, אייקון, כותרת ותיאור.
// כרגע משתמש ב-<a> פשוט — כשתוסיף react-router החלף ל-<Link>.
// ─────────────────────────────────────────────────────────

// מיפוי צבעים לפי prop — מונע hardcoding
const colorMap = {
  emerald: {
    bg: "bg-emerald-500",
    light: "bg-emerald-50",
...
/**
 * @param {{ to: string, color: "emerald"|"blue", icon: ReactNode, title: string, subtitle: string }} props
 */
const NavCard = ({ to, color, icon, title, subtitle }) => {
  ...
  return (
    ...
    >
      {/* אייקון */}
      <div className={`${c.light} ${c.text} p-3 rounded-xl flex-shrink-0 transition-colors`}>
        {icon}
      </div>
